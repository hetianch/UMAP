function out = JAABA_postprocessing_deleteproblemfly(goodtrx,allScores,skiplist)
%This function is used to post process problametic JAABA's output:
%1 add scores to goodtrx
%2 calculate agreement with Brad's prediction
%3 find discrepant frames and export to .txt

%% the difference between this function and JAABA_postprocessing_normal is that:
%when attach scores and look for diagnosis candidate this function skip the problametic
%flies.

[scoredTrx] = attachScores(goodtrx, allScores,skiplist);
[ diagnosiscandidate ] = findDiagCandidate(scoredTrx,goodtrx,skiplist);
...
function [scoredTrx] = attachScores(goodtrx, allScores,skiplist)
...
function [ diagnosiscandidate ] = findDiagCandidate(scoredTrx,goodtrx,skiplist)
...
%blank filled with a dummy variable -1 
