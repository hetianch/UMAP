function [ Xrescale ] = feature_scale(Xoriginal)
...
for i = 1:COL
    ...
    denominator=max(abs(Xoriginal(~isinf(Xoriginal(:,i)),i)));
...
max_after_rescale = max(abs(Xrescale(~isinf(Xrescale(:,k)),k)));
Xrescale(isinf(Xrescale(:,k)),k)=max_after_rescale*1000
