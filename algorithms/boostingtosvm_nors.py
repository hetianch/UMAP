...
data = numpy.loadtxt("/Users/071cht/Desktop/Lab/lab_data/JAABAtest3/multiflynewoutput/perframe/x_perframe_rescale.txt",delimiter=',') 
labels = numpy.loadtxt("/Users/071cht/Desktop/Lab/lab_data/JAABAtest3/multiflynewoutput/perframe/label_perframe.txt")
...
X_train = X_subset[0:seperateIdx] 
y_train = y[0:seperateIdx] 
X_test  = X_subset[seperateIdx:]
y_test  = y[seperateIdx:] 

print X_train.shape, X_test.shape

"""turn off feature selection for now Note feature selection only work for linear kernel"""
from sklearn.svm import SVC
from sklearn.feature_selection import RFECV
params = {'kernel': 'linear', 'probability': 1}
svc = SVC(**params)
rfecv = RFECV(estimator=svc, step=1, cv=6, scoring='accuracy')
svm_classifier=rfecv.fit(X_train, y_train)
...
params = {'kernel': 'linear','probability':1}
svc = SVC(**params)
#kfold = cross_validation.KFold(len(X_subset), n_folds=3)
#crossScores=[svc.fit(X_subset[train], y[train]).score(X_subset[test], y[test]) for train, test in kfold]
...
#gbc= GradientsvmClassifier(n_estimators=100, max_depth=1, learning_rate=1.0,random_state=0)
...
print 'svm_Precision:'+ ' '+ str(P_R_F[0])
print 'svm_Recall and sensitivity of the model:'+ ' '+ str(P_R_F[1])
print 'svm_F:'+ ' '+ str(P_R_F[2])
...
for i, y_pred_stage in enumerate(svm_classifier.staged_predict(X_test)):
...
for i, y_pred_stage in enumerate(svm_classifier.staged_predict(X_train)):
...
plt.ylabel('True')
plt.title ('confusion matrix for svm_linear')
