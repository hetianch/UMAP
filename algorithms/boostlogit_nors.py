...
data = numpy.loadtxt("/Users/071cht/Desktop/Lab/lab_data/multifly_data/data_reducefeature.txt",delimiter=',') 
labels = numpy.loadtxt("/Users/071cht/Desktop/Lab/lab_data/multifly_data/labels_reducefeature.txt")
...
crossScores=[logistic.fit(X[train], y[train]).score(X[test], y[test]) for train, test in kfold]
...
#sebset data for training and testing by random seed and use 60% to train and 40% to test
 #Seed the random number generator with a fixed value, so that it return the same array each time.
