...
k=np.linspace(1,21,11).astype(int)

for k_idx in range(11):
	knn_params={'n_neighbors':k[k_idx], 'weights': 'distance'}
	algorithm= neighbors.KNeighborsClassifier(**knn_params)
	ACC=list()
	RMSE=list()
	Precision=list()
	Recall=list()
	...
	for i,(train, test) in  enumerate (kfold):
		curr_classifier=algorithm.fit(X[train],Y[train])
