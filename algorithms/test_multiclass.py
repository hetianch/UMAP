...
path='/Users/071cht/Desktop/for_conference_longer/test0/output/gender/'
data = np.loadtxt(path+'gender_x_scale.txt',delimiter= ',') 
labels = np.loadtxt(path+'gender_y.txt')
...
#boost_params = {'n_estimators': 100, 'max_depth': 4,'learning_rate': 0.5, 'random_state': 0}
logit_params={'C':1e5}
svm_linear_params={'kernel': 'linear','probability':False}
#svm_gaussian_params={'kernel': 'rbf', 'probability': False}
#svm_linear_params={'C':1.0, 'class_weight':'None', 'dual':'True', 'fit_intercept':'True', 'intercept_scaling':1, 'loss':'l2', 'multi_class':'ovr','penalty':'l2','random_state':'None', 'tol':0.0001, 'verbose':0}
svm_gaussian_params={'C':1.0, 'cache_size':200, 'class_weight':'None', 'coef0':0.0, 'degree':3, 'gamma':0.0, 'kernel':'rbf','max_iter':-1,'probability':'False','random_state':'None','shrinking':'True','tol':0.001, 'verbose':'False'}
...
svm_linear=LinearSVC(C=1.0, class_weight=None, dual=True, fit_intercept=True,intercept_scaling=1, loss='l2', multi_class='ovr', penalty='l2',random_state=0, tol=0.0001, verbose=0)
svm_gaussian=SVC(C=1.0, cache_size=200, class_weight=None, coef0=0.0, degree=3,gamma=0.0, kernel='rbf', max_iter=-1, probability=False, random_state=0,shrinking=True, tol=0.001, verbose=False)
...
#algorithms_name=['svm with linear kernel','svm with gaussian kernel','k-nearest neighbors']
#algorithms_name=['svm with gaussian kernel']
for name in algorithms_name:
	...
	y_pred_whole=np.empty((Y.shape[0],),dtype=np.object)
	...
	pl.legend(loc="lower right",prop={'size':13})"""
	...
	#cm=confusion_matrix(Y,y_pred_whole)
	filename=name+'_pred_gender.mat'
	sio.savemat(path+filename,{'y_pred_gender':y_pred_whole})
